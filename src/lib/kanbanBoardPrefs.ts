/** Persisted Kanban board layout (per project). Task membership lives on each task. */

export type KanbanBoardSectionPersisted = {
  id: string;
  /** Empty string = use built-in i18n title for known ids */
  title: string;
  collapsed?: boolean;
};
...
/** Default section ids (Asana-style); titles from i18n when `title` is empty */
