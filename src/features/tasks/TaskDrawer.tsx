...
function TaskDrawerCreate({
  ...
  const { t } = useTranslation();
  const { addTask, nextDisplayNumber, tasks } = useTaskProject();
...
function TaskDrawerEdit({
  ...
  const { t } = useTranslation();
  const { patchTask, tasks } = useTaskProject();
  ...
  const overdueStatus = tab === 'task' ? draft.status : liveTask.status;
  const overdueDue = tab === 'task' ? draft.due_date : liveTask.due_date;
  const overdue =
    overdueStatus !== 'done' &&
    overdueStatus !== 'void' &&
    isDueOverdue(overdueDue.slice(0, 10));
  ...
  const chipType = tab === 'task' ? draft.type : liveTask.type;
  const chipTrade = tab === 'task' ? draft.trade : liveTask.trade;
  const chipPriority = tab === 'task' ? draft.priority : liveTask.priority;
  const chipDue = tab === 'task' ? draft.due_date : liveTask.due_date;
