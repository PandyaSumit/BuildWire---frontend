...
type Props = {
  ...
  /** When embedded in task detail, start/due are edited in the summary strip. */
  omitScheduleFields?: boolean;
};
    ...
    <div className="space-y-4">
      ...
      {!embedded && draft.status === "blocked" ? (
        <div>
          <label
            htmlFor={pf("blocked")}
            className="mb-1 block text-sm font-medium text-danger"
          >
            {t("newTaskDrawer.blockedReasonLabel")}{" "}
            <span className="text-danger">*</span>
          </label>
          <textarea
            id={pf("blocked")}
      ...
      {!omitScheduleFields ? (
        <div
          className={`grid grid-cols-1 gap-3 ${embedded ? "" : "sm:grid-cols-2"}`}
        >
          <div>
            <label
              htmlFor={pf("start")}
      ...
      <div className="rounded-xl border border-border/80 bg-surface/30 p-4">
        ...
        >
          <span className="text-sm font-medium text-primary">
            {t("taskForm.dropPhotos")}
          </span>
          <span className="mt-1 text-xs text-muted">
            {t("taskForm.uploadHint")}
          </span>
        </button>
      </div>
